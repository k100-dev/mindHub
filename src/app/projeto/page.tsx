  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_15%_5%,#d9faf2_0,transparent_25%),linear-gradient(180deg,#f8fbfb,#eef3f4)]">
      ...
      <section className="mx-auto grid max-w-7xl gap-5 px-6 pb-20 lg:grid-cols-2">
        ...
        <article className="flex flex-col justify-between rounded-[18px] bg-[#0e6e63] p-7 text-white md:p-8"><div><p className="text-xs font-extrabold uppercase tracking-[.14em] text-white/65">Próxima execução essencial</p><h2 className="mt-3 text-2xl font-black">Validar o Supabase de teste.</h2><p className="mt-3 text-sm leading-7 text-white/75">Aplicar a migração, criar contas fictícias e registrar a matriz de testes RLS. Mercado Pago e WhatsApp entram somente depois desse gate.</p></div><Link href="/p/dra-isadora-bezerra" className="button-secondary mt-7 self-start">Explorar demonstração <ArrowRight size={18} /></Link></article>
      </section>
    </main>
  );
}
