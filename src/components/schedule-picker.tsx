...
type ScheduleData = { slots: Slot[]; sessionDurationMinutes: number; depositAmount: number; sessionPrice: number; bookingEnabled: boolean };

function dayKey(value: string) { return new Intl.DateTimeFormat("en-CA", { timeZone: "America/Sao_Paulo" }).format(new Date(value)); }
...
export function SchedulePicker({ psychologistSlug, appointmentId, professional = false }: { psychologistSlug: string; appointmentId?: string; professional?: boolean }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/public/psychologists/${psychologistSlug}/slots?from=${encodeURIComponent(from.toISOString())}&to=${encodeURIComponent(to.toISOString())}${appointmentId ? `&appointmentId=${appointmentId}` : ""}`, { cache: "no-store", signal: controller.signal })
  ...
  }, [psychologistSlug, appointmentId]);
  ...
  async function reserve() {
    ...
    try {
      const response = await fetch(appointmentId ? `/api/appointments/${appointmentId}/manage` : "/api/appointments/holds", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(appointmentId ? { action: "RESCHEDULE", startsAt: selected } : { psychologistSlug, startsAt: selected }) });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error?.message ?? "O horário não está mais disponível.");
      router.push(`/${professional ? "app" : "hub"}/agendamentos/${body.appointment.id}`);
      router.refresh();
