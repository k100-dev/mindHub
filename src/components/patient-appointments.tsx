...
export function PatientAppointments({ appointments, now }: { appointments: Appointment[]; now: number }) {
  ...
  const upcoming = (item: Appointment) => new Date(item.ends_at).getTime() >= now && !["CANCELADO", "REALIZADO", "NAO_COMPARECEU", "NO_SHOW", "EXPIRADO", "REMARCADO"].includes(item.status);
